package com.mhd.interview.common.pipeline;
...
public final class BasicResultVO<T> implements Serializable {
    ...
    public BasicResultVO(ProcessResult status) {
        this(status, null);
    }

    public BasicResultVO(ProcessResult status, T data) {
        this(status, status.getMessage(), data);
    }

    public BasicResultVO(ProcessResult status, String msg, T data) {
     ...
     * @return 自定义状态的失败响应
     */
    public static <T> BasicResultVO<T> fail(ProcessResult status) {
     ...
     */
    public static <T> BasicResultVO<T> fail(ProcessResult status, String msg) {
